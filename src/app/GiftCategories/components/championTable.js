...
const headCells = [
  ...
  { id: 'cat_description', numeric: true, disablePadding: false, label: 'Detail', },
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    ...
                    <TableCell align="right" data-attr="">{row.description}</TableCell>
                    
                    <TableCell align="right" data-attr=""><Switch checked={ JSON.parse(row.status) == 1 ? true : false} 
                    inputProps={{ 'aria-label': 'controlled' }} onChange={()=>changeChampionStatus(row)}/></TableCell>										<TableCell align="right">{`${new Date(row.createdAt).getDate()}/${new Date(row.createdAt).getMonth()+1}/${new Date(row.createdAt).getFullYear()}`}</TableCell>
                    <TableCell align="right" data-attr=""><Link href={`GiftCategories/${row.catId}`}><span className="dashboard-table-icon"><BorderColorIcon /></span></Link></TableCell>
                  </TableRow>
                );
              })}
