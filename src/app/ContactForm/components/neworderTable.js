...
const headCells = [
  ...
  { id: 'cat_message',  numeric: false, disablePadding: false, label: 'Message', },
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div" style={{'fontSize':'22px','fontWeight:':'600'}}>  Queries List</Typography>
      )}
    </Toolbar>
  );
}
...
/**------download queries------ */
const csvData = [
  ["Name", "EMail", 'Mobile','Message', 'Status',  'Date',],
  ...rows.map(({ name, email, mobile,message, statusMessage, createdAt })=> [
    name, email, mobile,message, statusMessage, createdAt
  ]),
];
/**------download queries------ */
...
const fetchAllProjects=useCallback(()=>{
  ...
  .then((response) => { 
    ...
    console.log("Status value:", response.data.Data); // Add this line to log the status value
  ...
  return (
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        ...
        <div style={{textAlign:'right',marginTop:'10px', textTransform:'capitalize'}}><Button variant="outlined" style={{textTransform:'capitalize'}}><CSVLink data={csvData} filename={"Contact Queries.csv"}>Download Contact Queries</CSVLink></Button></div>
        <TableContainer>
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    tabIndex={-1} key={row.formId} selected={isItemSelected} sx={{ cursor: 'pointer' }} >
                    ...
                    <TableCell align="left" data-attr="">{row.message}</TableCell> 
