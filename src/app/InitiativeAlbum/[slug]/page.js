"use client"
import { useState, Suspense } from 'react'
import AddIcon from '@mui/icons-material/Add';
import Button from '@mui/material/Button';
import Loader from '@/app/component/Loader'
import Link from 'next/link'
import AlbumTable from '../components/albumTable'
export default function InitiativeAlbum({ params }){
  return(
            <div className="container">
                   <div className="row">
                     <div className="col-md-12">
                       <div className='page-header'>Album</div>
                     </div>
                     <div className="col-md-12" style={{marginBottom:'20px'}}>
                        <div className="row">
                            <div className="col-md-3">
                            </div>
                             <div className='col-md-7'></div>
                            <div className='col-md-2'>
                            {/* <Button variant="contained" color="error" style={{padding:'10px 20px', fontSize:'13px', marginLeft:'10px'}}><RiDeleteBinLine /> Delete</Button> */}
                            <Button variant="contained" color="success" style={{padding:'10px 20px', fontSize:'13px', marginLeft:'10px'}}>
                              <Link href={`/InitiativeAlbum/${params.slug}/addAlbum`} style={{color:'#fff'}}><AddIcon /> Add Album</Link></Button>
                            </div>
                        </div>
                     </div>
                     
                     <div className="col-md-12" style={{marginBottom:'150px'}}>
                     <Suspense fallback={<Loader/>}><AlbumTable albumId={params.slug}/></Suspense>
                     </div>
                    </div>
                </div>
  )
} 