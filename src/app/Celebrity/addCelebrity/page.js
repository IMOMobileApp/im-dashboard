...
export default function Addchampion(){
  ...
  async function uploadWithFormData() {
    pendingPopup()
     ...
     bodyContent.append("designation", desc);
     bodyContent.append("status", status);
     bodyContent.append("description", shotDesc);
     let response = await fetch(`${apiRoute}/addcelebrity`, { method: "POST", body: bodyContent, });
    ...
    function pendingPopup(){
      toastId.current =  toast.loading('Adding celebrity') }
    { data1.Status === true  ?    successPopup() : failPopup()}
    { data1.Status === true  ? router.push('/Celebrity') : ''}
    }
  /**----------------------------------------------------------------update blog--------------------------------------------- */
  /**---------------------------------------------------------------------------------------------------------------------------------------- */
      return(<>
                         ...
                         {/*-------------------------------------------------------------------------------------------------------------------------- */}
                          <div className="row">
                                <div className="col-md-4">
                                    <div className="input-head">Celebrity Name</div>
                                </div>
                              ...
                              <div className="row">
                                ...
                                <div className="col-md-8">
                                    <div className="input-field"><Textarea disabled={false} minRows={1} size="lg" variant="soft" placeholder="" value={desc} onChange={(e)=>{setDesc(e.target.value)}}
                                      style={{padding:'12px 15px',fontSize:'15px'}}/></div>
                                </div>
                              </div>
                              <div className="row">
                                <div className="col-md-4">
                                    <div className="input-head">Celebrity Image (500*500 px, square)</div>
                                </div>
                                <div className="col-md-8">
                              ...
                              <div className='row'>
                                ...
                                <Link variant="outlined" color="error" style={{width:'100%', fontSize:'15px',padding:'10px', display:'inline-block', textAlign:'center',border:'1px solid #000',color:'#000'}} href="/Celebrity">  Cancel </Link>
                                </div>
                                <div className='col-md-6'>
                                <Button variant="contained" color="success" style={{width:'100%', fontSize:'15px',padding:'10px'}} onClick={uploadWithFormData}> Add Celebrity </Button>
                                </div>
                              </div>
                                   {/*-------------------------------------------------------------------------------------------------------------- */}
                              </div>
