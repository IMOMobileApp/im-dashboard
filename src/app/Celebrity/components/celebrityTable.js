...
const headCells = [
  ...
  { id: 'cat_description', numeric: true, disablePadding: false, label: 'Designation', },
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div" >  Prominent Leaders </Typography>
      )}
    </Toolbar>
  );
}
...
export default function CelebrityTable() {
 ...
 // const visibleRows =  useMemo( () =>  rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage,),  [ page, rowsPerPage], );
  ...
  const changeChampionStatus = (event) => { 
    ...
    fetch(`${apiRoute}/updatecelebrity`, requestOptions)
      .then(response => response.text())
     // .then(result => console.log(result))
      .then(fetchAllChampionAPI) // fetch again all blog api after sending post request of changinh status
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    ...
                    <TableCell align="right" data-attr="">{row.designation}</TableCell>
                    
                    <TableCell align="right" data-attr=""><Switch checked={ JSON.parse(row.status) == 1 ? true : false} 
                    inputProps={{ 'aria-label': 'controlled' }} onChange={()=>changeChampionStatus(row)}/></TableCell>		
                    ...
                    <TableCell align="right" data-attr=""><Link href={`Celebrity/${row.celebId}`}><span className="dashboard-table-icon"><BorderColorIcon /></span></Link></TableCell>
                  </TableRow>
                );
              })}
