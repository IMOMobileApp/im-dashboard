...
const headCells = [
  ...
  { id: 'cat_name',  numeric: true, disablePadding: false, label: 'Ttile', },
  { id: 'cat_description', numeric: true, disablePadding: false, label: 'Category', },
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    ...
                    <TableCell padding="checkbox" data-attr=""><Checkbox color="primary"  checked={isItemSelected} inputProps={{ 'aria-labelledby': labelId, }}  onChange={(event) => handleClick(event, row._id)} sx={{ display: { xs: 'none', sm: 'none' } }}/></TableCell>
                    <TableCell align="right" data-attr="Image">
                    ...
                    <TableCell align="right" data-attr="Name">{row.blog_title}</TableCell>
                    <TableCell align="right" data-attr="Description">{row.short_desc}</TableCell>
                    
                    <TableCell align="right" data-attr="Published"><Switch checked={ JSON.parse(row.status) == 1 ? true : false} 
                    inputProps={{ 'aria-label': 'controlled' }} onChange={()=>changeBlogStatus(row)}/></TableCell>										<TableCell align="right" data-attr="Date">{`${new Date(row.createdAt).getDate()}/${new Date(row.createdAt).getMonth()+1}/${new Date(row.createdAt).getFullYear()}`}</TableCell>
                    <TableCell align="right" data-attr="Action"><Link href={`Blogs/${row.blogId}`}><span className="dashboard-table-icon"><BorderColorIcon /></span></Link></TableCell>
                  </TableRow>
                );
              })}
