...
const headCells = [
  ...
  { id: 'cat_description', numeric: false, disablePadding: false, label: 'Tree', },
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div" style={{'fontSize':'22px','fontWeight:':'600'}}>  Trees List</Typography>
      )}
    </Toolbar>
  );
}
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    ...
                    <TableCell align="left" data-attr="Description">{row.treeCount} / {row.totalTree}</TableCell>
                    <TableCell align="left" data-attr="Action">
                      {row.treeCount > 0 ? <Link href={`Trees/${row.projectId}`} style={{display:'block'}}><span className="dashboard-table-icon"><VisibilityIcon /></span></Link> :''}
