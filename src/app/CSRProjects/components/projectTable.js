...
const headCells = [
  ...
  { id: 'cat_sequence',  numeric: false, disablePadding: false, label: 'Sequence no.', },
  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div" style={{'fontSize':'22px','fontWeight:':'600'}}>CSR  Projects List</Typography>
      )}
    </Toolbar>
  );
}
...
export default function ProjectTable() {
 ...
 // const visibleRows =  useMemo( () =>  rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage,),  [ page, rowsPerPage], );
  ...
  const changeProjectStatus = (event) => { 
   ...
   bodyContent.append("proId", event.proId);
   ...
   bodyContent.append("userName", event.userName);
   bodyContent.append("userEmail", event.userEmail);
   bodyContent.append("userNumber", Math.floor(event.userNumber));
   bodyContent.append("type", event.type);
  ...
  return (
    ...
    <Box sx={{ width: '100%' }}>
      <Paper sx={{ width: '100%', mb: 2 }}>
        ...
        <TableContainer>
          <Table aria-labelledby="tableTitle"  size= 'medium' responsive="true" className='tabel-responsive' >
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    ...
                    <TableCell align="left" data-attr="Project Id">{row.projectId}</TableCell>
                    ...
                    <TableCell align="left" data-attr="Project Name">{row.name}</TableCell>
                    <TableCell align="left" data-attr="Project Name">{row.sequence}</TableCell>
                    <TableCell align="right" data-attr="Published" style={{textAlign:'left'}}> <Switch checked={ JSON.parse(row.status) == 1 ? true : false} 
                    inputProps={{ 'aria-label': 'controlled' }} onChange={()=>changeProjectStatus(row)} /></TableCell>
                    <TableCell align="left" data-attr="District">{row.district}</TableCell>
                    <TableCell align="left" data-attr="State">{row.state}</TableCell>
                    <TableCell align="left" data-attr="Pincode">{row.pincode}</TableCell>
                    <TableCell align="left" data-attr="Action"><Link href={`CSRProjects/${row.proId}`}><span className="dashboard-table-icon"><BorderColorIcon /></span></Link></TableCell>
                  </TableRow>
                );
              })}
