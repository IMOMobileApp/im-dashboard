 ...
 //   if (isLoading) return <Loader />
  ...
  async function uploadWithFormData() {
    pendingPopup()
     ...
     let response = await fetch(`${apiRoute}/addwebpress`, { method: "POST", body: bodyContent, });
    ...
    function pendingPopup(){
      toastId.current =  toast.loading('Adding Clips') }
    { data1.Status === true  ?    successPopup() : failPopup()}
    { data1.Status === true  ? router.push(`/initiativePressClipp/${params.slug}`) : ''}
...
{selectedImages ? selectedImages.name : ''}
 ...
 {/* -----------------------------------gallery---------------------------- */}
                              ...
                              <div className='row'>
                                ...
                                <div className='col-md-6'>
                                <Button variant="contained" color="success" style={{width:'100%', fontSize:'15px',padding:'10px'}} onClick={uploadWithFormData}> Add Clip </Button>
                                </div>
                              </div>
                                   {/*-------------------------------------------------------------------------------------------------------------- */}
                              </div>
