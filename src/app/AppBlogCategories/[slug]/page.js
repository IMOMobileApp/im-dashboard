...
export default function Caretakerdetail({ params }) {
  ...
  const fetchSpeciesDetail = () => {
    ...
    fetch(`${apiRoute}/detailappblogcat`, requestOptions)
      .then((response) => response.json())
      .then((result) => {
        setData(result.Data);
        setStatus(result.Data.status);
        setName(result.Data.catTitle);
        setSelectedImages(result.Data.catImage);
  ...
  async function uploadWithFormData() {
    ...
    let response = await fetch(
      `${process.env.NEXT_PUBLIC_API_ROUTE}/editappblogcat`,
