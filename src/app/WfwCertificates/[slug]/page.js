    ...
    const [isLoading, setLoading] = useState(true)
      useEffect(
        ()=>{
          ...
          var raw = JSON.stringify({  "userId": `${userId}`, "formId": params.slug });
          var requestOptions = { method: 'POST', headers: myHeaders, body: raw, redirect: 'follow'  };
          fetch(`${apiRoute}/waterChallengeFormDetail`, requestOptions)
          ...
          var raw = JSON.stringify({  "userId": `${userId}`, "formId": params.slug });
          var requestOptions = { method: 'POST', headers: myHeaders, body: raw, redirect: 'follow'  };
          fetch(`${apiRoute}/approveWaterCertificate`, requestOptions)
            .then(response => response.json())
            .then((result) => {
                  setData(result.Data)
                  setLoading(false)
          ...
          var raw = JSON.stringify({  "userId": `${userId}`, "formId": params.slug });
          var requestOptions = { method: 'POST', headers: myHeaders, body: raw, redirect: 'follow'  };
          fetch(`${apiRoute}/removeSpamCertify`, requestOptions)
            .then(response => response.json())
            .then((result) => {console.log(result)
                  //setData(result.Data)
                  setLoading(false)
                  router.push('/WfwCertificates')
                              })
          }
                     ...
                     <div className="row">
                      ...
                      <div className='row'>
                         ...
                         {/*-------------------------------------------------------------------------------------------------------------------------- */}
                                ...
                                <div className="col-md-4">
                                    <div className="input-field"><Input placeholder="" variant="soft" size="lg" style={{padding:'12px 15px',fontSize:'15px',color:'#000'}} value={data.phone} disabled  /></div>
                                </div>
                                ...
                                <div className="col-md-4">
                                    <div className="input-field">
                                        { data.isApproved == 0 ? 
                                        <><Button onClick={sendCertificate} variant="outlined" color="error" style={{width:'50%', fontSize:'15px',padding:'10px', display:'inline-block', textAlign:'center',border:'1px solid #000',color:'#000',textTransform:'capitalize'}}>
                                ...
                                <div className="col-md-4">
                                    <div className="input-field">
                                        <Image src={data.frameImage} alt="certificate" width={300} height={300}/>                                    </div>
                                </div>
                              ...
                              <div className='row'>
                                <div className='col-md-3'>
                                <Button variant="outlined" color="error" style={{width:'100%', fontSize:'15px',padding:'10px'}}> <Link href="/WfwCertificates" style={{color:'#d32f2f',width:'100%'}}>Back</Link> </Button>
                                </div>
