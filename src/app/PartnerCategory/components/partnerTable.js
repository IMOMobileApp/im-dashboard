  ...
  return (
    ...
    >
      {numSelected > 0 ? (
        ...
        <Typography sx={{ flex: '1 1 100%' }} variant="h6" id="tableTitle" component="div" >  Partners </Typography>
      )}
    </Toolbar>
  );
}
...
export default function PartnerTable() {
 ...
 // const visibleRows =  useMemo( () =>  rows.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage,),  [ page, rowsPerPage], );
  ...
  //   formdata.append("catName", event.catName); 
              ...
              {visibleRows.map((row, index) => {
                ...
                return (
                  <TableRow hover role="checkbox" aria-checked={isItemSelected}
                    tabIndex={-1} key={row.founderId} selected={isItemSelected} sx={{ cursor: 'pointer' }}>
                    <TableCell padding="checkbox" data-attr=""><Checkbox color="primary"  checked={isItemSelected} inputProps={{ 'aria-labelledby': labelId, }}  onChange={(event) => handleClick(event, row.founderId)}/></TableCell>
                   
                    <TableCell align="right" data-attr="">{row.catName}</TableCell>
                    <TableCell align="right" data-attr="">{row.sequence}</TableCell> 
                    ...
                    <TableCell align="right" data-attr="">{`${new Date(row.createdAt).getDate()}/${new Date(row.createdAt).getMonth()+1}/${new Date(row.createdAt).getFullYear()}`}</TableCell>
                    <TableCell align="right" data-attr=""><Link href={`PartnerCategory/${row.catId}`}><span className="dashboard-table-icon"><BorderColorIcon /></span></Link></TableCell>
                  </TableRow>
                );
              })}
